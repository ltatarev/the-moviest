...
import { ReviewType, WatchlistType } from '@lib/types';
import Image, { ImageProps } from 'next/image';
import { WatchlistCard } from './WatchlistCard';

type DetailsCardProps = {
  post: WatchlistType | ReviewType;
  imageProps: ImageProps;
};

export function DetailsCard({ post, imageProps }: DetailsCardProps) {
  if ((post as WatchlistType).description) {
    return (
      <WatchlistCard
        watchlist={post as WatchlistType}
        imageProps={imageProps}
      />
    );
  }
  ...
  const { title, movie, author_id, reviewText } = post as ReviewType;
  ...
  return (
    <div className="bg-neutral-200 select-none bg-opacity-80 flex flex-col lg:flex-row w-full lg:w-7/12 self-center rounded-xl p-10">
      ...
      <div className="flex flex-col lg:ml-10 mt-5">
        ...
        <p className="py-5 italic text-lg text-greyish">{reviewText}</p>
