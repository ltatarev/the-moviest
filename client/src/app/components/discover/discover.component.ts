...
export class DiscoverComponent implements OnInit {
  ...
  posterTv(tv) {
    ...
    newTv.poster_path = tv.results.map(tvs => tvs.poster_path = "https://image.tmdb.org/t/p/original" + tvs.poster_path)
    return newTv;
  }

  posterMovie(movie) {
    let newMovie = movie.results;
    newMovie.poster_path = movie.results.map(movies => movies.poster_path = "https://image.tmdb.org/t/p/original" + movies.poster_path)
    return newMovie;
  }
